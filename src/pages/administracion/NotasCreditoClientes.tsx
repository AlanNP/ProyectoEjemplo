...
import { MOCK_NOTAS_CREDITO_CLIENTES } from '../../data/mockData';
import type { NotaCredito } from '../../types';

const statusBadge: Record<string, string> = {
  vigente: 'badge-yellow',
  aplicada: 'badge-blue',
...
export default function NotasCreditoClientes() {
  const [data] = useState<NotaCredito[]>(MOCK_NOTAS_CREDITO_CLIENTES);

  const columns = [
    { key: 'folio', label: 'Folio', render: (row: NotaCredito) => (
      <span className="font-mono font-medium text-slate-700">{row.folio}</span>
    )},
    { key: 'empresa', label: 'Cliente', render: (row: NotaCredito) => (
      <div>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Notas de Crédito de Clientes"
        subtitle="Gestión de notas de crédito emitidas a clientes"
        icon={<FileX size={20} />}
        actions={
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="card text-center">
          <p className="text-3xl font-bold text-yellow-600">{data.filter(n => n.status === 'vigente').length}</p>
